...
type pkRound struct {
	...
	// 语义走，不是随便挑一个能压住刷屏的手段。
	//
	// 这条只堵死「同一个人刷屏」，堵不住「同一时刻不同的人一起涌入」
	// ——PK 接通瞬间对面 N 个不同的人各自第一次被欢迎，仍然是 N 条
	// 弹幕。第二道防线在 PkPanel.vue 的 `buildPkVisitRule`：给「内置/
	// PK串门欢迎」规则加了 `aggregate`（窗口内全部合并成一条 Trigger，
	// 跟 `buildEnterRule` 的进房欢迎同款做法），不在这里（曾经这里的
	// 注释错误地说这道防线"仍然保留"在 PkPanel.vue——那是终审第一轮
	// 复审时的真实状态，`buildPkVisitRule` 当时确实什么都没有，这句话
	// 是在描述一个不存在的东西；第二轮已经把 `aggregate` 真的加上）。
	//

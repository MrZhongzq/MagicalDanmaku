...
// callerPermissions 返回调用者在某绑定上拥有的权限点。
func (s *Server) callerPermissions(ctx context.Context, u *store.User, bindingID int64) ([]string, error) {
	if u.IsAdmin {
		return perm.Strings(perm.All()), nil
	}
	out := make([]string, 0, len(perm.All()))
	for _, p := range perm.All() {
		ok, err := s.store.Can(ctx, u.ID, bindingID, p)
		if err != nil {
			return nil, err
		}
		if ok {
...
func (s *Server) handleListBindings(w http.ResponseWriter, r *http.Request) {
	...
	for _, b := range bs {
		...
		ps, err := s.callerPermissions(r.Context(), u, b.ID)
		if err != nil {
			respondStoreError(w, err, "")
			return
		}
...
func (s *Server) handleCreateBinding(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, http.StatusCreated, bindingView{
		...
		RoomID: b.RoomID, Enabled: b.Enabled, RuleCount: 0, Permissions: ps,
	})
}

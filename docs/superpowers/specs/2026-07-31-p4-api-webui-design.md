...
- **账号所有者**——对自己账号下的全部绑定拥有**除 `member:manage` 外**的全部权限点
...
**为什么 `member:manage` 是例外。** 上面那个论证的形式是「严格弱于他已经握着的权力」，而所有者已有的权力**全是收缩性的**：删账号、删绑定、连带清空全部规则与授权。它们能清空别人的访问，不能凭空赋予一个新人访问。「能删光所有协作者」推不出「能新增一个协作者」——这是两个方向的操作，不是同一件事的强弱。把第三方拉进授权体系是管理员级别的决定，不是账号所有权的附带品。

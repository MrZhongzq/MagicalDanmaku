...
| 权限模型 | 按「账号-直播间」绑定授权，**不设固定角色，直接给权限点**（P3 确认） |
| 前端 | Vue3 + TypeScript，用 `embed.FS` 打进 Go 二进制（*建议，待 P4 确认*） |
| 数据库 | **只支持 PostgreSQL**（P3 确认，见 `2026-07-31-p3-data-layer-design.md`） |
| 分发目标 | win / darwin / linux × amd64 / arm64，加 Docker |
| 收费限制 | 全部移除 |

## 附录 B：保留与删除的功能清单

**保留**：弹幕接收与事件流、自动欢迎进场、礼物/关注/上舰答谢、关键词自动回复、定时任务、事件动作、自动禁言与永久禁言、弹幕记录与查询、多账号分工（**非轮换**，见 P2 设计）、数据统计看板、PK 大乱斗

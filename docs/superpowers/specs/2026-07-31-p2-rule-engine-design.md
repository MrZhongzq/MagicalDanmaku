...
| 层 | 作用 | 归属 | 实现 |
|---|---|---|---|
| 账号限流 | 防账号触发风控 | `account.Account` | P0 的 `ratelimit.Limiter` |
| 冷却组 | 多规则共享节流 | `rules.Cooldown` | 组名 → 上次触发时间 |
| 规则冷却 | 单规则最小间隔 | `rules.Cooldown` | 规则名 → 上次触发时间 |

### 8.1 为什么账号限流不在 Cooldown 里

`rules.Cooldown` 是**绑定内**的组件：每个「账号-直播间」一个实例。
而防风控的发送限流是**账号级**的——同一账号连接的全部直播间必须共享
发送节奏，否则账号 A 在三个房间各发各的，实际频率是配置值的三倍。

因此限流器挂在 `account.Account` 上，由 `account.Binding` 在真正发送前
等待。`rules.Executor` **不再重复等待**：两处都等会让实际间隔翻倍。

### 8.2 检查顺序

规则冷却与冷却组由 `Cooldown.Allow` 在触发前检查，**任一不通过即跳过
本次触发**（不排队、不延迟发送）；账号限流则在动作真正发送时阻塞等待。
...
理由：每个绑定的规则是使用者刻意选定的。让另一个账号顶替，等于执行
它从未被配置去做的动作——轻则行为出乎意料（本不该发言的账号开始发言），
重则直接失败（小号没有房管权限，禁言动作必然报错）。

注意这里**不存在能力限制机制**：所谓「主播号不发言」只是它的绑定下没有
配置任何发言规则，而非系统禁止它发言。是否发言完全由配置决定。
